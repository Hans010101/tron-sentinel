...
def fetch_trx_price_history() -> list:
    """
    Call CoinGecko /coins/tron/market_chart for the 7-day price history.

    Returns a list of [timestamp_ms, price_usd] pairs (hourly granularity).
    Returns an empty list on any error so callers can degrade gracefully.
    """
    logger.info("Fetching TRX 7-day price history from CoinGecko…")
    try:
        raw = _http_get(_CHART_URL)
        prices = raw.get("prices", [])
        logger.info("TRX price history: %d data points", len(prices))
        return prices
    except Exception as exc:
        logger.warning("Could not fetch TRX price history: %s", exc)
        return []


def fetch_trx_market() -> dict:
    """
    Call CoinGecko /coins/tron (and /market_chart for 7-day history) and
    return a market_data row dict.

    Dict keys:
        coin, source, price_usd, change_24h, market_cap, volume_24h,
        extra_json, collected_at

    extra_json includes: market_cap_rank, last_updated, ath_usd,
                         price_history ([[ts_ms, price], …])
    """
    ...
    price_history = fetch_trx_price_history()
